...
def loadcols(infile, format='', pl=0, sep=None, labels=None):
...
##     words = string.split(line)
    ...
    if labels == None:
        ...
        for i, label in enumerate(labels):
            exec('out.%s = array(data[i])' % label)
            #exec('out.%s = data[i]' % label)
        return out
...
def loaddata(filename, dir="", silent=0, headlines=0, cols=None):
...
def loadmachine(filename, dir="", silent=0):
    ...
    while line[0] <> '-':
        ...
        if 0:
            ...
            cols.append(xx)
        col1 = string.strip(line[1:4])
        col2 = string.strip(line[5:8])
        if col1:
            xx.append(int(col1))
            xx.append(int(col2))
        else:
...
class Cat2D:
    ...
    def assigndata(self):
        ...
        exec('self.%s = self.z = self.data[1:,1:]' % self.labels[2])
    def getold(self, x, y, dointerp=0):
        ix = interp(x, self.x, arange(len(self.x)))
        iy = interp(y, self.y, arange(len(self.y)))
        if not dointerp:  # JUST GET NEAREST
            #ix = searchsorted(self.x, x)
    ...
    def get(self, x, y, dointerp=0):
        if not dointerp:  # JUST GET NEAREST
            ...
            if 0:
                ...
                print self.y.shape
                print self.z.shape
            z = bilin3(x, y, self.x, self.y, self.z)
        return z
...
class VarsClass:
    def __init__(self, filename='', dir="", silent=0, labels=[], labelheader='', headlines=0, loadheader=1, cols=None):
        self.header = ''
        self.descriptions = {}
        try:
            ny, nx = filename.shape  # Array?
            self.name = ''
            self.data = filename
            self.labels = labels
            self.assigndata()
            ...
            if filename:
                if strend(filename, '.fits'):  # FITS TABLE
                    ...
                    self.data = loaddata(filename, dir, silent, headlines, cols)
                    # NOTE header IS global, GETS READ IN BY loaddata
                    if loadheader:
                        self.header = header or labelheader
                    if header:
                        labelheader = labelheader or header[-1]
                    self.labels = labels or string.split(labelheader)[1:]
                    # Look for SExtractor-style descriptions
                    for headline in header:
                        if headline[:2] == '##':
                            continue
                        ...
                        if len(words) < 4:
                            continue
                        pound, num, label = words[:3]
                        num = str2num(num)
                        if type(num) <> int:
                            continue
                        i = headline.find(label) + len(label)
                        self.descriptions[label] = headline[i:].strip()
                    # self.labels = string.split(header[-1][1:])
                    # self.labelstr = string.join(self.labels, ', ')[:-2]
                    self.assigndata()
            else:
                self.name = ''
                self.labels = []  # Otherwise carries over from last time for some reason!
                #self.labels = labels
                if len(labels):
                    for label in labels:
                        self.assign(label, array([]))
                    self.updatedata()
        self.formats = {}
        self.units = {}
        self.notes = []
    ...
    def assigndata(self):
        if len(self.data) == 0:
            self.data = array([[]] * len(self.labels))
    ...
    def updateddata(self):
        ...
        try:
            ...
            print 'Data lengths may not match:'
            for label in self.labels:
                data1 = self.get(label)
                print len(data1), label
            exec('data3 = array([%s])' % selflabelstr)
        return data3
    def updatedata(self):
...
##             print "VarsClass: SUBSET CANNOT BE CREATED: good LENGTH = %d, data LENGTH = %d" % (len(self.id), len(good))
    ...
    def greater(self, labels, lo):
        return self.between(lo, labels, None)
    def less(self, labels, hi):
        return self.between(None, labels, hi)
    def closeto(self, labels, val, tol=0.001):
        """labels = list of labels or just one label"""
        return self.between(val-tol, labels, val+tol)
    def equal(self, *other):
        return self.closeto(*other)
    ...
    def putids(self, label, ids, values, idlabel='id', rep=True):
        ...
        if len(ids) <> len(values):
            print "ERROR! Lengths of ids & values don't match in putids.  Returning."
            return
        selfid = self.get(idlabel).astype(int) # [6 4 5]
        maxselfid = max(selfid)
        if max(ids) > maxselfid:
            print "WARNING! Max id requested %d is greater than max id %d in catalog in putids." % (max(ids), maxselfid)
            good = less(ids, maxselfid+1)
            #print good
            #print ids
            #print values
            ids = compress(good, ids)
            values = compress(good, values)
        #idstochange = set(ids)
        exec('x = self.%s.copy()' % label)
        idchecklist = selfid.copy()
        #print 'idchecklist\t', idchecklist
        done = False
        while not done: # len(idstochange):
            indexlist = zeros(maxselfid+1, int) - 1
            put(indexlist, idchecklist, arange(self.len()))  # [- - - - 1 2 0]  # id[0] = 6, etc.
            indices = take(indexlist, array(ids).astype(int))  # ids = [4 6]  ->  indices = [1 0]
            ...
            if -1 in indices:
                if rep < 2:
                    print "WARNING! NOT ALL IDS FOUND IN putids!",
                    print compress(less(indices, 0), ids)
                    good = greater(indices, -1)
                    ids = compress(good, ids)
                    values = compress(good, values)
                    indices = compress(good, indices)
                elif total(indices) == -len(indices):
                    break
            put(x, indices, values)
            ...
            if 0:
                print 'x\t', x[:10]
                print 'id\t', ids[:10]
                print 'indexlist\t', indexlist[:10]
                #print idsdone[:10]
                print 'idchecklist\t', idchecklist[:10]
                print len(x)
    ...
    def add(self, label, data, format=None, description=None):
        if 1:  #self.labels:
            if singlevalue(data):
		if self.len():
		    data = zeros(self.len(), float) + data
    ...
    def append(self, self2, over=0):
        ...
        if labels <> labels2:
            ...
            self.updatedata()
        return self
    def merge(self, self2, labels=None, replace=0, checkids=1):
    ...
    def addids(self):
        if 'id' not in self.labels:
            ids = arange(self.len()) + 1
            self.assign('id', ids)
            self.labels.insert(0, self.labels.pop())  # move id to front of labels
...
##             pause()
    ...
    def findmatches2a(self, searchcat, dtol=0):
        ...
        self.assign('dist', d)
    def findmatches2(self, searchcat, dtol=0, xylabels=['x', 'y']):
        """Finds closest matches for self within searchcat"""
        if xylabels == 'Sky':
            xlab, ylab = 'RA', 'Dec'
        elif xylabels == 'sky':
            ...
            xlab, ylab = xylabels
        i, d = findmatches2(searchcat.get(xlab), searchcat.get(ylab), self.get(xlab), self.get(ylab))
        if dtol:
            i = where(less(d, dtol), i, -1)
        self.assign('matchi', i)
        self.assign('dist', d)
    def findmatches2xy(self, x, y, dtol=0, xylabels=['x', 'y']):
        """Finds closest matches for self within searchcat"""
        if xylabels == 'Sky':
            xlab, ylab = 'RA', 'Dec'
        elif xylabels == 'sky':
            ...
            xlab, ylab = xylabels
        i, d = findmatches2(x, y, self.get(xlab), self.get(ylab))
        if dtol:
            i = where(less(d, dtol), i, -1)
        self.assign('matchi', i)
        self.assign('dist', d)

    def convertcoords(self, infits, outfits):
        xycat = self.copy()
        xycat.labels = string.split('x y')
        xycat.header = '.'
        inxy = 'tempin.xy'
        outxy = 'tempout.xy'
        xycat.save(inxy)
        convertcoords.convertcoords(infits, inxy, outfits, outxy)
        newxycat = loadcat('tempout.xy', labels=xycat.labels)
        self.rename('x', 'xold')
        self.rename('y', 'yold')
        self.assign('x', newxycat.x)
        self.assign('y', newxycat.y)
    ...
    def rename(self, oldlabel, newlabel):
        ...
        if self.formats:
            if oldlabel in self.formats.keys():
                self.formats[newlabel] = self.formats[oldlabel]
        if self.units:
    ...
    def save(self, name='', dir="", header='', format='', labels=1, pf=0, maxy=300, machine=0, silent=0):
        ...
        if not format:
            if self.formats:
                format = ' '
                for label in self.labels:
                    if label not in self.formats.keys():
                        print 'Missing %s key in format' % label
                        format = ''
                        break
                    fmt = self.formats[label]
                    format += fmt + '  '
                if format:
                    format = format[:-2] + '\n'
    ...
    def pr(self, header='', more=True, pf=0, format='', labels=1):  # print
        labelsorig = self.labels[:]
        self.save('tmp.cat', silent=True, header=header, pf=pf, format=format, labels=labels)
        self.labels = labelsorig[:]
        if more:
...
def loadvarswithclass(filename, dir="", silent=0, labels='', header='', headlines=0, loadheader=1, cols=None):
    """INPUT: CATALOG w/ LABELED COLUMNS
    OUTPUT: A CLASS WITH RECORDS NAMED AFTER EACH COLUMN
    >>> mybpz = loadvars('my.bpz')
    >>> mybpz.id
    >>> mybpz.data -- ARRAY WITH ALL DATA"""
    outclass = VarsClass(filename, dir, silent, labels, header, headlines, loadheader, cols)
    #outclass.assigndata()
    return outclass
...
def prunecols(infile, cols, outfile, separator=None):
...
def loadsexcat0(infile, dir='', silent=0):
    """Loads a file of SExtractor format into a VarsClass catalog
    Grabs full label names from lines with single comments, e.g.,
    # 1	radius_pix
    # 2	radius_asec
    # 3	EE_F435W
    
    Output catalog will have cat.radius_pix , etc.
    """
    ...
    for headline in header:
        ...
        if ncols == 1:
            ...
            for icol in range(ncols):
                #params.append(param+'[%d]' % icol)
                params.append(param+'_%d' % icol)
        lastcol = col

    cat = loadcat(infile, labels=params)
    return cat
...
class SExSegParamsClass:
    ...
    def save(self, name='', header=''):
        ...
        for param in self.params:
            line = ''
            comment = self.comments.get(param, '')
            if not comment:
                line += param
            else:
    ...
    for iy in range(ny):
        ...
        for ix in range(nx):
            try:
                ...
                print "iy, ix, nx, len(ss), ss, ss[ix]"
                print iy, ix, nx, len(ss)
                print ss
                print ss[ix]
                die()
...
def saveregions(x, y, filename, labels=[], x2=None, y2=None, precision=1, coords='image', color="green", symbol="circle", size=0, width=0):
    ...
    for i in range(n):
        if x2 <> None:
            sout = 'line %s %s %s %s' % (x[i], y[i], x2[i], y2[i])
        else:
            if not size and not width:
...
def saveregions3(x, y, filename, labels=[], precision=1, coords='image', color="green", symbol="circle", size=0, width=0):
    """SAVES POSITIONS AND LABELS AS A ds9 REGIONS FILE"""
    fout = open(filename, 'w')
    fout.write('global color=' + color + ' font="helvetica 10 normal" select=1 edit=1 move=1 delete=1 include=1 fixed=0 source\n')
    #fout.write("image\n")
...
def saveregions2(x, y, filename, labels=[], precision=1, coords='image', color="green", symbol="circle", size=None, width=None):
    """SAVES POSITIONS AND LABELS AS A ds9 REGIONS FILE"""
    fout = open(filename, 'w')
    fout.write('global color=' + color + ' font="helvetica 10 normal" select=1 edit=1 move=1 delete=1 include=1 fixed=0 source\n')
    #fout.write("image\n")
...
def savelabels(x, y, labels, filename, coords='image', color="green", symbol="circle", precision=1, xyprecision=None, fontsize=12, bold=1):
    """SAVES POSITIONS AS A ds9 REGIONS FILE"""
    if coords == 'sky':
        coords = 'fk5'
	...
	label = "%%.%df" % precision % label
        ...
        if xyprecision == 's':
            fout.write("text %s %s {%s}\n" % (x[i], y[i], label))
        elif xyprecision == None:
            fout.write("text %g %g {%s}\n" % (x[i], y[i], label))
        else:
            xstr = "%%.%df" % xyprecision % x[i]
            ystr = "%%.%df" % xyprecision % y[i]
            fout.write("text %s %s {%s}\n" % (xstr, ystr, label))
    fout.close()
...
def fitsval(filename, keyword):
    filename = capfile(filename, '.fits')
    f = pyfits.open(filename, memmap=1)
    return f[0].header.get(keyword, None)
...
def loadpixelscale(image):
	...
	pixelscale = 0
    elif s[i-4:i] == 'bits': # -32 bits/pixel
	print 'PIXEL SCALE NOT FOUND IN IMAGE HEADER OF', capfile(image, '.fits')
	pixelscale = 0
    else:  # -0.0500/0.0500s/pix
	s = string.split(s, '/pix')[0][:-1]  # remove trailing s or "
	s = string.split(s, '/')[1]
	pixelscale = string.atof(s)
    os.remove('temp.txt')
    return pixelscale
