...
def quadratic(a, b, c):
    """Solutions to the quadratic formula a*x**2 + b*x + c = 0"""
    pos = (-b + sqrt(b**2 - 4*a*c)) / (2 * a)
    neg = (-b - sqrt(b**2 - 4*a*c)) / (2 * a)
    return pos, neg

def dang1(ang1, ang2, unit='deg'):
    """Difference between two angles"""
    half = {'deg':180, 'rad':pi}[unit]
    d = ang2 - ang1
    while d > half:
        return dang(ang1, ang2-2*half, unit=unit)
    while d < -half:
        return dang(ang1-2*half, ang2, unit=unit)
    return abs(d)

def dang(ang1, ang2, unit='deg', half=None):
    """Difference between two angles"""
    if half == None:  # maybe you want to set it to half this for shears
        half = {'deg':180, 'rad':pi}[unit]
    ang1 = array(ang1)
    ang2 = array(ang2)
    d = ang2 - ang1
    ...
    #print 'ang2', ang2
    #print 'd', d
    if (min(d) < -half) or (max(d) > half):
        ang1 = where(less(d, -half),   ang1-2*half, ang1)
        ang2 = where(greater(d, half), ang2-2*half, ang2)
        return dang(ang1, ang2, unit=unit)
    else:
        return abs(d)

def dang2(ang1, ang2, unit='deg'):
    """Difference between two angles"""
    half = {'deg':90, 'rad':pi/2.}[unit]
    ang1 = array(ang1)
    ang2 = array(ang2)
    d = ang2 - ang1
    #print 'ang1', ang1
    ...
    if (min(d) < -half) or (max(d) > half):
        ang1 = where(less(d, -half),   ang1-2*half, ang1)
        ang2 = where(greater(d, half), ang2-2*half, ang2)
        return dang2(ang1, ang2, unit=unit)
    else:
        return abs(d)

#dang2([0, 350, 90, 180], [359, 0, 777, 0])
...
def logspace2(lo,hi,num=50):
    return(logspace(log10(lo), log10(hi), num=num))
...
def stringsplitatof(str, separator=''):  # coetools.py
    if separator:
...
def hms2h(x):
    """
    CONVERTS hours : minutes : seconds to decimal hours
    """
    h, m, s = stringsplitatof(x, ':')
    hout = abs(h) + m/60. + s/3600.
    sgn = sign(h)
    if sgn == 0:
        sgn = 1
    hout = hout * sgn
...
def percentile(p, x):
    """Value v at percentile p in x"""
    x = sort(x)
    i = p * (len(x) - 1.)
    v = interp(i, arange(len(x)), x)
    return v

def percentile2(v, x):
    """Percentile of value v in x"""
    p = searchsorted(sort(x), v) / float(len(x))
    return p
...
def bilin3(x, y, xdata, ydata, zdata):  # --DC
    """Return z(x,y) given zdata(xdata,ydata)
    Use bi-linear interpolation"""
    outgrid = RectBivariateSpline(ydata,xdata,zdata,kx=1,ky=1)
    if singlevalue(x):
        if not singlevalue(y):
            x = x * ones(len(y))
    elif singlevalue(y):
        y = y * ones(len(y))
    z = outgrid.ev(y, x)
...
def meangeom2(m):  # Equivalent
    return 10**mean(log10(abs(m)))
...
def plothistobiggles(a,da=1.,amin=[],amax=[]): # --DC
    p = FramedPlot()
