...
# Or could just use "phr in str"
def strin(str, phr):
    return str.find(phr) > -1
...
def findbestmatch(x, xsearch):
    """RETURNS THE INDEX OF x WHERE xsearch IS CLOSEST"""
    return argmin(abs(array(x)-array(xsearch)))

def findbestmatches(x, xsearch):
    ii = []
    for x1 in xsearch:
        ii.append(findbestmatch(x, x1))
    
    return array(ii)

def findbestmatch2(x, y, x1, y1):
    """Finds closest match for (x1,y1) in (x,y)"""
    dx = x - x1
    dy = y - y1
    d = hypot(dx, dy)
    i = argmin(d)
...
def findmatches1(x, xsearch, tol=0):
    ii = []
    for x1 in xsearch:
        ii.append(findmatch1(x, x1, tol=tol))
...
# use bpztypename below
def bpzsedname(tb, seds, interp=2, clip=False):  # clipping doesn't always give what you want
    if type(seds) == str:
    ...
    if abs(rb - tb) > 0.1:
        rb2 = roundint((tb - rb) * (interp + 1.) + rb)
        name2 = seds[rb2-1]
        if clip:
            name2 = string.split(name2, '_')[0]
            name = name + '-' + name2
        else:
            name = name[:-4] + '-' + name2
    return name

def bpztypename(tb, tbs, space=0, fracs=0):
    rb = roundint(tb)
    name = tbs[rb-1].strip()
    if name[-4:] == '.sed':
        name = name[:-4]
    if abs(rb - tb) > 0.009:
        ...
        if fracs:
            ...
            f2 = abs(tb - rb)
            spaces = space*' '
            name = '%.2f' % f1 + spaces + '*' + spaces + name
            name += spaces + '+' + spaces
            name += '%.2f' % f2 + spaces + '*' + spaces + name2
        else:
            name += space*' ' + '-' + space*' ' + name2
    return name
